...
def get_loaders(train_path, val_path, size, batch_size: int, fp16: bool = True, val_batch_size=None,
                num_workers: int = 12, rect_val=False, min_scale=0.08, distributed=False):
    """
    ...
    train_dataset = datasets.ImageFolder(train_path, transforms.Compose(train_transforms))
...
def create_validation_set(valdir, batch_size, target_size, rect_val, distributed):
    ...
    val_transforms = [transforms.Resize(int(target_size * 1.14)),
